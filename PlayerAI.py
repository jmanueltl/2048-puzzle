...
class PlayerAI(BaseAI):
    ...
    def __init__(self):
        ...
        self.emptyWeight = 2.9
        self.alpha = -PlayerAI.infinity
        self.beta = PlayerAI.infinity
        self.maxWeight = 1.0
        self.distanceWeight = 10.0

    def evaluate_heuristic(self, grid):

        empty_cells = len(grid.getAvailableCells())
        max_tile = grid.getMaxTile()

        smoothness = self.smoothness(self, grid) * self.smoothnessWeight
        ...
        for x in xrange(grid.size):
            ...
            for y in xrange(grid.size):
                if max_tile == grid.map[x][y]:
                    ...
                    if max_tile < 1024:
                        distan = -((abs(x - 0) + abs(y - 0)) * max_tile)
                    else:
                        distan = -((abs(x - 0) + abs(y - 0)) * (max_tile / 3))
                    break
...
#Algorithm
    def alpha_beta(self, grid, alpha, beta, depth, player):

        if time.clock() - start > self.time_limit:
            return self.evaluate_heuristic(grid), -1, True

        if depth == 0:
            return self.evaluate_heuristic(grid), -1, False
        ...
        if player:
            ...
            if len(positions) == 0:
                return self.evaluate_heuristic(grid), None, False
            ...
            for position in positions:
                ...
                score, move, timeout = self.alpha_beta(new_grid, alpha, beta, depth - 1, False)
            ...
            for cell in cells:
                ...
                score, move, timeout = self.alpha_beta(new_grid, alpha, beta, depth - 1, True)
    ...
    def iterative(self, grid):
        ...
        while True:
            ...
            score, move, timeout = self.alpha_beta(grid, self.alpha, self.beta, depth, True)
